...
#用训练完的caffemodel计算测试集的mae,分别测试20~60岁，20~40岁的
import caffe
...
def main():
    ...
    for it in range(niter):
        ...
        for batch in range(0,20):#根据batchsize修改
            if solver.test_nets[0].blobs['labels'].data[batch, 0, 0, 0].T >= 20 and solver.test_nets[0].blobs['labels'].data[batch, 0, 0, 0].T <= 40:
        ...
        print("age=", age)
        mae=mae/countrange#根据batchsize修改
        mae_all=(mae_all*(count-countrange)+mae*countrange)/count
